...
# App bundle creation script / 앱 번들 생성 스크립트
APP_NAME="GeoTagger"
...
# Remove existing bundle / 기존 번들 제거
rm -rf "$BUNDLE_DIR"

# Create directory structure / 디렉토리 구조 생성
mkdir -p "$MACOS_DIR"
mkdir -p "$RESOURCES_DIR"

# Copy executable file / 실행 파일 복사
cp "GeoTagger/.build/arm64-apple-macosx/release/$APP_NAME" "$MACOS_DIR/"

# Copy Python script / Python 스크립트 복사
cp "geotag.py" "$RESOURCES_DIR/"

# Create Info.plist / Info.plist 생성
...
# Create icon (using SF Symbols) / 아이콘 생성 (SF Symbols 사용)
...
# Set execute permissions / 실행 권한 설정
chmod +x "$MACOS_DIR/$APP_NAME"

# Code signing / 코드 서명
codesign --force --deep --sign - "$BUNDLE_DIR"

echo "✅ App bundle creation complete / 앱 번들 생성 완료: $BUNDLE_DIR"
echo "📱 Run app in Finder / Finder에서 앱 실행: open $BUNDLE_DIR"